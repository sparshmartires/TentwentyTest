...
const SeatRow: React.FC<SeatRowProps> = ({ row, seats, selectedSeats, toggleSeatSelection, seatTypes }) => {
  return (
    <View style={styles.row}>
      {seats.map((seat, index) => {
        const seatId = `${row}-${index}`;
        const isSelected = selectedSeats.includes(seatId);
        const seatImage = isSelected ? seatTypes[1].image : seatTypes[seat].image;
